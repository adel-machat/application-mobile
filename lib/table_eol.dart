...
class tableEOL extends StatefulWidget {
  const tableEOL({Key? key}) : super(key: key);

  @override
  State<tableEOL> createState() => _tableEOLState();
}

class _tableEOLState extends State<tableEOL> {
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < jsonResponse.length; i++) {
        ...
        dataList.add({
          'ieol': jsonResponse[i]['ieol'].toString(),
          'peol': jsonResponse[i]['peol'].toString(),
    ...
    for (var row in _data) {
      ...
      if (rowDate == date) {
        filteredRows.add(DataRow(
          cells: [
            DataCell(Text(row['ieol'].toString(),style: TextStyle(fontSize: 16),)),
            DataCell(Text(row['peol'].toString(),style: TextStyle(fontSize: 16),)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Historiques de éolienne'),
      ),
      body: Center(
                    ...
                    child: selectedDate != null
                      ...
                      rows: _data.map((jsonResponse) {
                        return DataRow(cells: [
                          DataCell(Text(jsonResponse['ieol'].toString(),style: TextStyle(fontSize: 16),)),
                          DataCell(Text(jsonResponse['peol'].toString(),style: TextStyle(fontSize: 16),)),
