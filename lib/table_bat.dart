...
class tableBatt extends StatefulWidget {
  const tableBatt({Key? key}) : super(key: key);

  @override
  State<tableBatt> createState() => _tableBattState();
}

class _tableBattState extends State<tableBatt> {
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < jsonResponse.length; i++) {
        ...
        dataList.add({
          'ibatt': jsonResponse[i]['ibatt'].toString(),
          'pbatt': jsonResponse[i]['pbatt'].toString(),
    ...
    for (var row in _data) {
      ...
      if (rowDate == date) {
        filteredRows.add(DataRow(
          cells: [
            DataCell(Text(row['ibatt'].toString(),style: TextStyle(fontSize: 16),)),
            DataCell(Text(row['pbatt'].toString(),style: TextStyle(fontSize: 16),)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Historiques de battrie'),
      ),
      body: Center(
                    ...
                    child: selectedDate != null
                      ...
                      rows: _data.map((jsonResponse) {
                        return DataRow(cells: [
                          DataCell(Text(jsonResponse['ibatt'].toString(),style: TextStyle(fontSize: 16),)),
                          DataCell(Text(jsonResponse['pbatt'].toString(),style: TextStyle(fontSize: 16),)),
