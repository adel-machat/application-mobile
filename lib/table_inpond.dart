...
class tableIN extends StatefulWidget {
  const tableIN({Key? key}) : super(key: key);

  @override
  State<tableIN> createState() => _tableINState();
}

class _tableINState extends State<tableIN> {
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < jsonResponse.length; i++) {
        ...
        dataList.add({
          'iintond': jsonResponse[i]['iintond'].toString(),
          'pintond': jsonResponse[i]['pintond'].toString(),
    ...
    for (var row in _data) {
      ...
      if (rowDate == date) {
        filteredRows.add(DataRow(
          cells: [
            DataCell(Text(row['iintond'].toString(),style: TextStyle(fontSize: 16),)),
            DataCell(Text(row['pintond'].toString(),style: TextStyle(fontSize: 16),)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Historiques d`entré de l`onduleur'),
      ),
      body: Center(
                    ...
                    child: selectedDate != null
                      ...
                      rows: _data.map((jsonResponse) {
                        return DataRow(cells: [
                          DataCell(Text(jsonResponse['iintind'].toString(),style: TextStyle(fontSize: 16),)),
                          DataCell(Text(jsonResponse['pintond'].toString(),style: TextStyle(fontSize: 16),)),
