...
class tableTemp extends StatefulWidget {
  const tableTemp({Key? key}) : super(key: key);
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < jsonResponse.length; i++) {
        ...
        dataList.add({
          'temp': jsonResponse[i]['temp'].toString(),
          'hum': jsonResponse[i]['hum'].toString(),
    ...
    for (var row in _data) {
      ...
      if (rowDate == date) {
        filteredRows.add(DataRow(
          cells: [
            DataCell(Text(row['temp'].toString(),style: TextStyle(fontSize: 16),)),
            DataCell(Text(row['hum'].toString(),style: TextStyle(fontSize: 16),)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Historiques de temp'),
      ),
      body: Center(
                  ...
                  child: Container(
                    ...
                    child: selectedDate != null
                      ...
                      child: Text('No data found.',style: TextStyle(fontSize: 30),),
                        ...
                        : DataTable(
                      columns: const [
                        DataColumn(label: Text('température',style: TextStyle(fontSize: 20),)),
                        DataColumn(label: Text('humidité',style: TextStyle(fontSize: 20),)),
                      ...
                      rows: _data.map((jsonResponse) {
                        return DataRow(cells: [
                          DataCell(Text(jsonResponse['temp'].toString(),style: TextStyle(fontSize: 16),)),
                          DataCell(Text(jsonResponse['hum'].toString(),style: TextStyle(fontSize: 16),)),
